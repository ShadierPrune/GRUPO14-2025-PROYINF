  ...
  useEffect(() => {
    ...
    const fetchSimulacion = async () => {
      try {
        ...
        if (!response.ok)
          throw new Error("Error al obtener la simulación de la API");
        ...
        const tasaEfectiva = tasa / 100;
        const cuotaLocal =
          tasaEfectiva > 0
            ? (monto * (tasaEfectiva * Math.pow(1 + tasaEfectiva, plazo))) /
              (Math.pow(1 + tasaEfectiva, plazo) - 1)
            : monto / plazo;
